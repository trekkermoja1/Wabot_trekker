    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"📊 Status for {instance_id}: {data.get('status')} - Code: {data.get('pairingCode')}")
                return data
...
async def create_instance(request: CreateInstanceRequest):
    async with db_pool.acquire() as conn:
        ...
        if existing:
            ...
            if not target_server:
                raise HTTPException(status_code=400, detail=f"All servers are full.")
    ...
    return InstanceResponse(
        ...
        status="new", server_name=target_server, 
        created_at=datetime.utcnow().isoformat(), port=port
    )
...
async def get_pairing_code(instance_id: str):
    async with db_pool.acquire() as conn:
        ...
        url = f"http://127.0.0.1:{port}/pairing-code"
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                response = await client.get(url)
                if response.status_code == 200:
                    data = response.json()
                    return {"pairing_code": data.get("pairingCode"), "status": data.get("status")}
        except Exception as e:
            print(f"❌ Error polling pairing-code API: {e}")
...
async def stop_instance(instance_id: str):
    if instance_id in bot_processes:
        try:
            ...
            bot_processes[instance_id].kill()
        del bot_processes[instance_id]
        return {"message": "Instance stopped"}
    
    # Try to find and kill if not in dictionary
    try:
        async with db_pool.acquire() as conn:
            pid = await conn.fetchval("SELECT pid FROM bot_instances WHERE id = $1", instance_id)
            if pid:
                subprocess.run(['kill', str(pid)])
                await conn.execute("UPDATE bot_instances SET pid = NULL WHERE id = $1", instance_id)
                return {"message": "Instance killed via PID"}
    except: pass
    
    return {"message": "Instance was not running"}
...
async def list_instances(status: Optional[str] = None, id: Optional[str] = None):
    async with db_pool.acquire() as conn:
        ...
        for instance in instances:
            ...
            result.append({
                "id": instance['id'], "name": instance['name'], "phone_number": instance['phone_number'],
                "status": status_data.get("status", instance['status']),
                "server_name": instance['server_name'], "owner_id": instance['owner_id'],
                "port": instance['port'], "pairing_code": status_data.get("pairingCode"),
